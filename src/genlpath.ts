...
const generateById = async (id: string): Promise<string> => {
    ...
    const files = await fs.readdir(`usersSubmission_records/${id}`)

    for (const file of files) {
        const submissions = JSON.parse(await fs.readFile(`usersSubmission_records/${id}/${file}`, 'utf-8'))
...
const init = async () => {
    ...
    const fHandler: fs.FileHandle = await fs.open('learning.csv', 'w')
